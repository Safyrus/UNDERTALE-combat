...
def append_byte(textbin, val, name, i):
    if val > 255:
        printv(f"WARNING: {name} is > 255 (val={val}) at {i}. Replacing by 0", param="tw", v=V_WARNING)
        val = 0
    textbin.append(val)
    return textbin


def add_normal_char(textbin: bytearray, c):
    marker = None
...
def txt_2_bin(text):
    # filtering file
    printv(f"filtering...", param="t", v=V_INFO)
    ...
    # find labels
    printv(f"parsing... (labels)", param="t", v=V_INFO)
    textbin = bytearray()
    i = 0
    labels = {}
    consts = {}
    while i < len(text):
        ...
        if c == "<":
            ...
            if name == "label":
                ...
                printv(f"label: '{args[0]}' at {hex(labels[args[0]])}", param="it", v=V_DEBUG)
            elif name == "jump":
                ...
                printv(f"const: '{args[0]}' with value '{args[1]}'", param="it", v=V_DEBUG)
    ...
    # parsing file
    printv(f"parsing... (all)", param="t", v=V_INFO)
    textbin = bytearray()
    i = 0
    while i < len(text):
        c = text[i]
        if c == "<":
            ...
            if name == "end":
                ...
                if args[0] not in labels:
                    adr = 0xFFFFF
                    printv(f"WARNING: Unknow label '{args[0]}'. address has been replaced by {adr}", param="wt", v=V_WARNING)
                else:
                    adr = labels[args[0]]
                c = 0
                n = 0
    ...
    printv("text size:", len(textbin), param="t", v=V_INFO)
...
if __name__ == "__main__":
    ...
    # read text file
    printv(f"reading file...", param="t", v=V_INFO)
    with open(txtfile, "r", encoding="utf-8") as f:
        text = f.read()

    #
    textbin = txt_2_bin(text)

    # outputting results
    printv(f"writing new file...", param="t", v=V_INFO)
